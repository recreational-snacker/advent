...
public class Advent2018Day10Test {

    private String testInput = "position=< 9,  1> velocity=< 0,  2>\n" +
    ...
    private String input ="position=<-31667, -52902> velocity=< 3,  5>\n" +
            ...
            "position=<-31661, -52899> velocity=< 3,  5>";

    @Test
    public void testPart1Visualisation() {
        ...
        for (String s : inputArray) {
            ...
            String x = s1.split(", ")[0].trim();
            String y = s1.split(", ")[1].trim();
            String s2 = s.split("velocity=<")[1].split(">")[0];
            String xVel = s2.split(", ")[0].trim();
            String yVel = s2.split(", ")[1].trim();
            points.put(x + "," + y + "," + 1, xVel + "," + yVel);
            if (Integer.parseInt(x) > maxx) {
                maxx = Integer.parseInt(x);
            }
            if (Integer.parseInt(y) > maxy) {
                maxy = Integer.parseInt(y);
            }
        }

        int timesLooping = 1;
        while (timesLooping < 5) {
            System.out.println(points.size());
            System.out.println(timesLooping+" :----------------------------------");
            for (int y = maxy; y > maxy * -1; y--) {
                StringBuilder row = new StringBuilder();
                for (int x = maxx * -1; x < maxx; x++) {
                    String velocity = points.get(x + "," + y + "," + timesLooping);
                    if (velocity != null) {
                        row.append("#");
                        int xVel = Integer.parseInt(velocity.split(",")[0]) * timesLooping;
                        int yVel = Integer.parseInt(velocity.split(",")[1]) * timesLooping;
                        int xCoord = x + xVel;
                        int yCoord = y + yVel;
                        int loop = timesLooping+1;
                        points.put(xCoord+","+yCoord+","+loop, velocity);
                    } else {
                        row.append(".");
                    }
                }
