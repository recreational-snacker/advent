...
public class Day3Test {
    ...
    private String testInput1 = "R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51\n" +
            "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7";

    private String testInput2 = "R8,U5,L5,D3\n" +
            "U7,R6,D4,L4";

    String[] wire1coords;
    String[] wire2coords;
    int steps;
    Map<String, Integer> stepsTaken = new HashMap<>();

    @Test
    public void problem1() {
        Map<String, String> coords = getCoordMapForWires(input);
        Integer minDistance = null;
        for (Map.Entry<String, String> entry : coords.entrySet()) {
           if (entry.getValue().equalsIgnoreCase("X")) {
               Integer calculateDistance = calcDistance(entry.getKey());
               if (minDistance == null) {
                   minDistance = calculateDistance;
               } else if (calculateDistance < minDistance) {
                   minDistance = calculateDistance;
               }
           }
        }
    ...
    public void problem2() {
        Map<String, String> coords = getCoordMapForWires(input);
        steps=0;
        mapWire(1, wire1coords, coords);
        steps=0;
        mapWire(2, wire2coords, coords);
        System.out.println(Collections.min(stepsTaken.values()));
    }

    private void extractArrays(String input) {
        String[] wires = input.split("\n");
        String wire1 = wires[0];
        String wire2 = wires[1];
        wire1coords = wire1.split(",");
        wire2coords = wire2.split(",");
    }

    private Map<String, String> getCoordMapForWires(String input) {
        extractArrays(input);
    ...
    private Integer calcDistance(String key) {
    ...
    private void mapWire(Integer wire, String[] wireCoords, Map<String, String> coords) {
        Integer startX = 0;
        Integer startY = 0;
        for (int i = 0; i < wireCoords.length; i++) {
            String direction = wireCoords[i].substring(0,1);
            Integer length = Integer.valueOf(wireCoords[i].substring(1));
            if (direction.equalsIgnoreCase("R")) {
                startX = handleRight(wire, startX, startY, length, coords);
            } else if (direction.equalsIgnoreCase("L")) {
                startX = handleLeft(wire, startX, startY, length, coords);
            } else if (direction.equalsIgnoreCase("U")) {
                startY = handleUp(wire, startX, startY, length, coords);
            } else if (direction.equalsIgnoreCase("D")) {
                startY = handleDown(wire, startX, startY, length, coords);
            }
        }

    }

    private Integer handleRight(Integer wire, Integer startX, Integer startY, Integer length, Map<String, String> coords) {
        Integer y = startY;
        Integer x = startX;
        for (int j = 1; j <= length; j++) {
            ...
            if(coords.containsKey(coord)) {
                String wireAlreadyThere = coords.get(coord);
                if (!wire.toString().equalsIgnoreCase(wireAlreadyThere) && !wireAlreadyThere.equalsIgnoreCase("X")) {
                    coords.remove(coord);
                    coords.put(coord, "X");
                } else if (wireAlreadyThere.equalsIgnoreCase("X")) {
                    if (stepsTaken.containsKey(coord)) {
                        stepsTaken.put(coord, stepsTaken.get(coord)+steps);
                    } else {
                        stepsTaken.put(coord, steps);
                    }
                }
            } else {
                coords.put(coord, wire.toString());
            }
        }
        return x;
    }

    private Integer handleLeft(Integer wire, Integer startX, Integer startY, Integer length, Map<String, String> coords) {
        Integer y = startY;
        Integer x = startX;
        for (int j = 1; j <= length; j++) {
            steps++;
    ...
    private Integer handleUp(Integer wire, Integer startX, Integer startY, Integer length, Map<String, String> coords) {
        Integer y = startY;
        Integer x = startX;
        for (int j = 1; j <= length; j++) {
            steps++;
    ...
    private Integer handleDown(Integer wire, Integer startX, Integer startY, Integer length, Map<String, String> coords) {
        Integer y = startY;
        Integer x = startX;
        for (int j = 1; j <= length; j++) {
            steps++;
