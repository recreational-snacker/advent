...
public class AdventDay4 {
    ...
    public void testPart1() {
        String[] linesToProcess = input.split("\\n");
        Integer validPassphrases = 0;
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                for (int j = 0; j < list.size(); j++) {
                    if (j != i) {
                        ...
                        if (compareAnagrams(word, compare)) {
                            valid = false;
                        }
                    }
                }
    ...
    private boolean compareAnagrams(String word, String compare) {
        char[] wordArray = word.toLowerCase().toCharArray();
        char[] compareArray = compare.toLowerCase().toCharArray();
        Arrays.sort(wordArray);
        Arrays.sort(compareArray);
        return Arrays.equals(wordArray, compareArray);
    }
}
