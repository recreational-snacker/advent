...
public class Day4Test {
    ...
    private String sample = "ecl:gry pid:860033327 eyr:2020 hcl:#fffffd\n" +
    ...
    private List<String> splitPassportList(String input) {
    ...
    private List<String> getMandatoryFields(String exclusion) {
        List<String> mandatoryFields = new LinkedList<>(Arrays.asList("byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid", "cid"));
        mandatoryFields.remove(exclusion);
        return mandatoryFields;
    }

    private void countValidPassports(String input, String exclusion) {
        int validPassportsSet1 = 0;
        int validPassportsSet2 = 0;
        List<String> passports = splitPassportList(input);
        for (String passport : passports) {
            List<String> mandatoryFields = getMandatoryFields(exclusion);
            String[] details = passport.split(" ");
            for (String detail : details) {
                String field = detail.split(":")[0];
        ...
        System.out.println("Valid passports set 1: "+validPassportsSet1);
        System.out.println("Valid passports set 2: "+validPassportsSet2);
    }

    private boolean validatePassport(String data) {
        ...
        if (key.equalsIgnoreCase("iyr")) {
            if (!validateIssueYear(value)) {
                return false;
            }
        }
        if (key.equalsIgnoreCase("eyr")) {
    ...
    private boolean validateBirthYear(String data) {
        Integer birthYear = Integer.parseInt(data);
        return birthYear <= 2002 && birthYear >= 1920;
    }

    private boolean validateIssueYear(String data) {
        Integer issueYear = Integer.parseInt(data);
        return issueYear <= 2020 && issueYear >= 2010;
    }

    private boolean validateExpiryYear(String data) {
        Integer issueYear = Integer.parseInt(data);
        return issueYear <= 2030 && issueYear >= 2020;
    }

    private boolean validateHeight(String data) {
        if (data.contains("cm")) {
            Integer height = Integer.parseInt(data.replace("cm", ""));
            return height >= 150 && height <= 193;
        } else {
            Integer height = Integer.parseInt(data.replace("in", ""));
            return height >= 59 && height <= 76;
        }
    }

    private boolean validateEyeColour(String data) {
        List<String> eyeColour = new LinkedList<>(Arrays.asList("amb", "blu", "brn", "gry", "grn", "hzl", "oth"));
        return eyeColour.contains(data);
    }

    private boolean validateHairColour(String data) {
        Pattern pattern = Pattern.compile("#([a-f0-9]{6})$");
        Matcher matcher = pattern.matcher(data);
        return data.length() == 7 && matcher.lookingAt();
    }

    private boolean validatePassportId(String data) {
        Pattern pattern = Pattern.compile("\\d{9}");
        Matcher matcher = pattern.matcher(data);
        return matcher.lookingAt() && data.length() == 9;
    }

    @Test
    public void test() {
        countValidPassports(sample, "cid");
        countValidPassports(input, "cid");
    }

    private String input = "hgt:159cm\n" +
