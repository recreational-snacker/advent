...
public class Day1Test {
    ...
    public void test1() {
        ...
        for (String s : input.split("\n")) {
            ...
            if (map.size()==1) {
                int cal = Integer.parseInt(map.get(0).toString()+map.get(0).toString());
                calibration+=cal;
            }
            if (map.size()==2) {
                int cal = Integer.parseInt(map.get(0).toString()+map.get(1).toString());
                calibration+=cal;
            }
            if (map.size()>2) {
                int cal = Integer.parseInt(map.get(0).toString()+map.get(map.size()-1).toString());
                calibration+=cal;
            }
        }
        ...
        for (String s : input.split("\n")) {
            ...
            while (sIndex < s.length()) {
                ...
                for (Value value : Value.values()) {
                    try {
                        String o = s.substring(sIndex, sIndex + value.length);
                        if (o.equalsIgnoreCase(value.name())) {
                            map.put(index, value.val);
                            index++;
                            sIndex += value.length - 2;
                            break;
                        }
                    } catch (Exception e) {
    ...
    enum Value {
        ...
        Value(int val, int length) {
            this.val = val;
            this.length = length;
        }
    }

    private String sample = "two1nine\n" +
            "eightwothree\n" +
            "abcone2threexyz\n" +
            "xtwone3four\n" +
            "4nineeightseven2\n" +
    ...
    private String input = "eighttkbtzjz6nineeight\n" +
